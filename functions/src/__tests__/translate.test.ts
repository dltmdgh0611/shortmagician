...
const handler = translate as unknown as (req: any) => Promise<any>;

// ── Helpers ─────────────────────────────────────────────────────────────

function seg(id: string, text: string) {
  return {id, start_time: Number(id), end_time: Number(id) + 1, text};
}

function batchReply(translations: string[], tokens = 100) {
  return {
...
function singleReply(translation: string, tokens = 50) {
  return {
