...
// ── Imports ─────────────────────────────────────────────────────────────
const synthesize_1 = require("../pipeline/synthesize");
const handler = synthesize_1.synthesize;
// ── Helpers ─────────────────────────────────────────────────────────────
...
describe("synthesize", () => {
    ...
    it("rejects unauthenticated requests", async () => {
        await expect(handler({ data: { text: "hi", voice_id: "v", language: "en" } })).rejects.toMatchObject({ code: "unauthenticated" });
    });
    // ─ Text validation ────────────────────────────────────────────────
    ...
    it("rejects text exceeding 5000 chars", async () => {
        await expect(handler(validReq({ text: "a".repeat(5001) }))).rejects.toMatchObject({ code: "invalid-argument" });
    });
    // ─ Language validation ────────────────────────────────────────────
    it("rejects unsupported language code", async () => {
        await expect(handler(validReq({ language: "xx" }))).rejects.toMatchObject({ code: "invalid-argument" });
    });
    ...
    it("returns base64-encoded audio on success", async () => {
        ...
        expect(result.audioBase64).toBe(Buffer.from(fakeAudio).toString("base64"));
    });
    ...
    it("clamps speed to 0.25–4.0 range", async () => {
        ...
        expect(mockSynthesizeSpeech.mock.calls[0][0].input.ssml).toContain('rate="25%"');
        // Too high → 400%
        mockSynthesizeSpeech.mockResolvedValueOnce([{ audioContent: fakeAudio }]);
        await handler(validReq({ speed: 10 }));
        expect(mockSynthesizeSpeech.mock.calls[1][0].input.ssml).toContain('rate="400%"');
    });
