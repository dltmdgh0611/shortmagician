...
// ── GPT splitting ──────────────────────────────────────────────────────────
function buildSplitPrompt(text, n, language) {
    ...
    return (`You are inserting ${n - 1} line break(s) into subtitle text, creating ${n} segments.\n` +
...
async function callOpenAISplit(prompt) {
    ...
    return client.chat.completions.create({
        ...
        messages: [
            {
                role: "system",
                content: "You are a subtitle line-break specialist. You decide WHERE to insert " +
...
function parseSplitResponse(raw) {
    if (!raw)
        return null;
    try {
...
const CLAUSE_BOUNDARY = /(?<=[,;:\uff0c\uff1b\uff1a\u3001])\s+|\s+(?=(?:and|but|so|or|yet|because|although|however|while|when|if|since|after|before|unless|until|though|whereas)\s)/i;
const CJK_SENTENCE_BOUNDARY = /(?<=[。！？\uff01\uff1f])/;
const CJK_CLAUSE_BOUNDARY = /(?<=[、，；\u3001\uff0c\uff1b])/;
function mergeToN(parts, n, sep = " ") {
    const result = [...parts];
...
function splitCjkFallback(text, n) {
...
function splitTextFallback(text, n) {
...
function ensureMaxWidth(text, maxWidth, out, depth = 0) {
    ...
    const cjk = isCjkDominant(text);
    if (depth >= 5) {
        console.warn(`  max_width sub-split depth exceeded for: ${text.slice(0, 40)}...`);
        out.push(text);
        return;
    }
    ...
    console.warn(`  Part too wide (${pw} > ${maxWidth}), sub-splitting into ${subN} (depth=${depth})`);
...
// ── Core split logic ───────────────────────────────────────────────────────
async function splitTextWithGpt(text, targetChunks, language) {
    const cjk = isCjkDominant(text);
    ...
    try {
        ...
        if (parts) {
            ...
            if (parts.length > targetChunks) {
                ...
                console.warn(`GPT returned ${parts.length} parts instead of ${targetChunks} — merged tail`);
                parts = merged;
            }
            else if (parts.length < targetChunks) {
                console.warn(`GPT returned ${parts.length} parts instead of ${targetChunks} — using as-is`);
            }
            ...
            if (originalNorm === joinedNorm) {
                ...
                console.warn("GPT split dropped/altered content — falling back to algorithmic split\n" +
...
exports.splitSegments = (0, https_1.onCall)({ memory: "512MiB", timeoutSeconds: 120, secrets: [openaiApiKey] }, async (request) => {
    ...
    console.info(`Split request: ${segments.length} segments (max_duration=${maxDur}s, max_lines=${maxLines})`);
    ...
    for (const seg of segments) {
        ...
        const chunksByDuration = duration > maxDur ? Math.ceil(duration / maxDur) : 1;
        const targetChunks = Math.max(chunksByWidth, chunksByDuration);
        console.info(`  Splitting seg id=${seg.id}: ${duration.toFixed(1)}s, width=${width} → ${targetChunks} chunks`);
        // Split text via GPT
        const { parts: rawParts, tokens } = await splitTextWithGpt(text, targetChunks, body.target_language);
        totalTokens += tokens;
    ...
    console.info(`Split complete: ${segments.length} → ${resultSegments.length} segments, ${totalTokens} tokens`);
    return { segments: resultSegments };
});
//# sourceMappingURL=splitSegments.js.map